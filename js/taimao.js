...
function GetToken() {
    if ($request.url.indexOf("/api/") > -1 && $request.body) {
        let reqBody = $request.body;
        if (reqBody.indexOf("Token") > -1 && reqBody.indexOf("11641") > -1) {
            let oldBody = $.getdata(signBodyKey);
            if (oldBody !== reqBody) {
                $.setdata(reqBody, signBodyKey);
                $.msg($.name, "获取打卡数据成功 🎉", "太茂专属通行证已保存！可以去测试啦。");
            }
        }
    }
    $.done();
...
function checkIn() {
    ...
    if (!bodyStr) {
        $.msg($.name, "签到失败 ⚠️", "未找到数据，请先去微信小程序里随便点点获取凭证。");
        $.done();
        return;
    }
    ...
    $.post(request, (error, response, data) => {
        try {
            if (error) {
                ...
                // 提取返回的提示信息
                let msg = (obj.d && obj.d.Msg) ? obj.d.Msg : (obj.e || "");
                let title = (obj.d && obj.d.Title) ? obj.d.Title : "";

                // 【核心优化】增加对 2054 和文本中包含"已签到"的识别
                if (obj.m === 1 || msg.includes("成功")) {
                    $.msg($.name, "签到成功 🎉", msg + (title ? "\n" + title : ""));
                } else if (obj.m === 2054 || msg.includes("已经签到") || msg.includes("已签到")) {
                    $.msg($.name, "今日已签到过啦 ☕️", msg);
                } else if (obj.m === 0) {
                    $.msg($.name, "签到提示", msg || "请查看日志");
                } else {
                    $.msg($.name, "签到提示", msg || "状态未知，请查看日志详细返回数据");
                    console.log("金桥太茂返回数据：" + data);
                }
            }
        } catch (e) {
            $.msg($.name, "解析失败", "服务器返回数据异常");
            console.log(e);
        } finally {
            $.done();
        }
