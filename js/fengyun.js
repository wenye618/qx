...
[rewrite_local]
^https:\/\/applet\.njqsmx\.com\/mobile\/business\/sign\/.* url script-request-body fengyun.js

[task_local]
40 9 * * * fengyun.js, tag=风云再起签到, img-url=https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/mini/Game.png
...
function checkIn() {
    ...
    if (!url || !bodyStr) {
        $.msg($.name, "签到失败 ⚠️", "未找到数据，请先去微信小程序里，真实点击一次“签到/打卡”按钮。");
        $.done();
        return;
    }
    ...
    $.post(request, (error, response, data) => {
        try {
            if (error) {
                ...
                // 【核心优化】深入 header 和 body 解析风云再起的特殊结构
